...
class ConRanFieldFcams(nn.Module):
    def __init__(self, weight, sigma_rgb, sigma_xy, scale_factor, device):
        super(ConRanFieldFcams, self).__init__()

        self.loss = DenseCRFLoss(
            weight=weight, sigma_rgb=sigma_rgb,
            sigma_xy=sigma_xy, scale_factor=scale_factor
        ).to(device)
