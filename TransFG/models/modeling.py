...
class Encoder(nn.Module):
    ...
    def get_attention(self, hidden_states):
        attn_weights = []
        for layer in self.layer:
            hidden_states, weights = layer(hidden_states)
            attn_weights.append(weights)

        x = hidden_states#self.norm(x)
...
class Transformer(nn.Module):
    ...
    def get_attention(self, input_ids):
        embedding_output = self.embeddings(input_ids)
        x, x1, attn_weights = self.encoder.get_attention(embedding_output)

        ttn_weights = torch.stack(attn_weights)  # 12 * B * H * N * N
        attn_weights = torch.mean(ttn_weights, dim=2)  # 12 * B * N * N

        feature_map = x1.detach().clone()  # B * C * 14 * 14
        b = feature_map.shape[0]
        w = h = int(math.sqrt(feature_map.shape[1]))

        cams = attn_weights[1:4].sum(0)[:, 0, 1:].reshape(b, h, w)
...
class VisionTransformer(nn.Module):
    ...
    def get_attention(self, x, labels=None):
        cams = self.transformer.get_attention(x)
        return cams
