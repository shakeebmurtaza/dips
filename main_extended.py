...
import utils
import vision_transformer_extended as vits
# from vision_transformer import DINOHead
from wsol_fusion_eval_newsampling_extended.data_loaders import get_data_loader_non_dist as get_data_loader
from wsol_fusion_eval_newsampling_extended.data_loaders_overlapped import get_data_loader_non_dist as get_data_loader_overlapped
from wsol.wsol_utils import str2bool, configure_data_paths, configure_log_folder

from wsol_fusion_eval_newsampling_extended.inference import CAMComputer
# from wsol.inference import CAMComputer as CAMComputer_for_six_heads
# from sam_optimizer.sam import SAM
import copy
# from efficientnet_pytorch import EfficientNet
# from filelock import FileLock
from dlib_extended.unet_with_class_head.model import Unet
from dlib_extended.unet_with_class_head.decoder import DecoderBlock
...
from wsol_fusion_eval_newsampling_extended.inference import normalize_tensor
...
def get_args_parser():
    ...
    parser.add_argument('--seed_tech', type=str, default='uniform')#,
                        # choices=(self_learning._WEIGHTED, self_learning._UNIFORM))
    parser.add_argument('--seed_tech_bg', default='uniform',)# type=str, default=self_learning._UNIFORM,
                        # choices=(self_learning._WEIGHTED, self_learning._UNIFORM))
    parser.add_argument('--classifier_for_top1_top5', type=str, default=None,
                        choices=('TransFg', 'Dino_Head', 'fusionnet'))
...
def evaluate_fusionmodule(epoch, split, model, classifier, loaders, fusioscorenet, args, evaluate_on_MaxBoxV2 = False):
    ...
    if args.classifier_for_top1_top5 != None and args.dataset_name != 'OpenImages':
...
def train_fusionmodule(args, trial):
    ...
    # fusioscore_net = Unet(encoder_name=args.unet_encoder_name, in_channels=fusion_channels, classes=fusion_module_out_channels, classification_head_classes = num_labels).cuda()#FusionScores().cuda()
    fusioscore_net = LocalizationHead(dim=teacher.embed_dim, num_labels=fusion_module_out_channels).cuda()
    ...
    if args.use_classifier or args.classifier_for_top1_top5 != None:
        if (not args.use_dino_classifier) and (not args.evaluate):
            ...
            if args.dataset_name == 'ILSVRC':
                ...
                torch_load_path_cub_cls_lyer = '/home/emursha/DGA1032_code/dino_orginal/linear_classifier_on_CUB_dataset_patch_16-pretrained_dino/checkpoint.pth.tar' if args.dino_pretrained == '' else '/home/emursha/DGA1032_code/dino_orginal/linear_classifier_on_CUB_dataset_patch_16/checkpoint.pth.tar'
...
def train_one_epoch(teacher, data_loader, optimizer, lr_schedule,
    ...
    for it, (images, raw_images, target, _, roi, head_indx) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        if args.seed_type == 'MBProbSeederSLCamsWithROI':
            ...
            mean_attention = torch.max(attentions, dim=1).values.unsqueeze(1)
        ...
        ############################Get CAMs############################
        with torch.no_grad():
            x_out, attentions = teacher.forward_get_all_heads(images)
            x_cls, x_patch = x_out[:, 0], x_out[:, 1:]
            n, p, c = x_patch.shape
            x_patch = torch.reshape(x_patch, [n, int(p**0.5), int(p**0.5), c])
            x_patch = x_patch.permute([0, 3, 1, 2])
            x_patch = x_patch.contiguous()
        cam_fused = fusioscore_net(x_patch)
        # print('wait')
        ############################Get CAMs############################

        # cam_fused, class_logits = fusioscore_net(images.detach())
        ...
        loss = loss_seed
        # loss_cls = class_loss(class_logits, target)
...
if __name__ == '__main__':
    ...
    if args.evaluate or args.run_for_given_setting or args.evaluate_only_on_attetion_maps:
        if os.path.exists(args.evaluate_checkpoint) or args.evaluate_only_on_attetion_maps or args.run_for_given_setting:
            if os.path.exists(args.evaluate_checkpoint):
                keys_to_ignore = ['evaluate_checkpoint', 'data_root', 'metadata_root', 'run_for_given_setting', 'mask_root']
                ...
                # keys_to_ignore = ['use_classifier', 'use_dino_classifier', 'crf_fc', 'num_samples_to_save', 'mask_root', 'evaluate_only_on_attetion_maps', 'exp_pre_name', 'classifier_for_top1_top5', 'run_accumulate_and_save_fun', 'proxy_training_set', 'evaluate', 'run_for_given_setting', 'evaluate_checkpoint', 'data_root']
                keys_to_ignore = ['run_accumulate_and_save_fun', 'use_dino_classifier', 'classifier_for_top1_top5', 'iou_regions_path', 'num_samples_to_save', 'mask_root', 'exp_pre_name', 'proxy_training_set', 'evaluate', 'run_for_given_setting', 'evaluate_checkpoint', 'data_root', 'output_dir']
                keys_to_ignore = ['classifier_for_top1_top5', 'mask_root', 'exp_pre_name', 'proxy_training_set', 'evaluate',  'evaluate_checkpoint', 'data_root', 'cam_curve_interval', 'output_dir', 'use_dino_classifier']
                with open(os.path.join(args.evaluate_checkpoint, 'args.txt')) as json_file:
                    ...
                    for key in args_tmp.keys():
                        if 'dino_pretrained' == key and args_tmp[key] == '':
                            ...
                            print('ignoring', key, 'and replacing with:', getattr(args, key))
                    print('loaded exp setting for exp', args.evaluate_checkpoint)
            ...
            args.log_folder = configure_log_folder(args)
            args.output_dir = args.log_folder
            print('will save log to:', args.log_folder)
        ...
        if args.search_hparameters or args.run_for_best_setting:
            ...
            if args.search_hparameters:   
                ...
                if os.path.exists(comet_config_file_path):
                    ...
                    if args.top_n_head_for_sampling > 0:
                        paramters["top_n_head_for_sampling"] = {"type": "discrete", "values": [1, 3, 5, 7]} if args.discrete_search_space else {"type": "integer", "min": 1, "max": 10}
